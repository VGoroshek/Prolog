...
clauses
    onFileNew(_Source, _MenuTag) :-
        ...
        Filename = vpiCommonDialogs::getFileName("*.txt", ["Текстовый файл", "*.txt"], "Открыть базу данных", [], Path, _),
        !,
        Db = dbrel::new(Filename),
        Db:load(),
        dbrel::currentDb := some(Db),
        Form = familyForm::display(This),
        Form:setText(Filename).

    onFileNew(_Source, _MenuTag).
        % This code is maintained automatically, do not update it manually. 11:54:13-27.2.2018
